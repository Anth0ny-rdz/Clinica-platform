...
export async function fetchAppointmentsCalendarByDoctor(doctorId: number) {
  const res = await fetch(`${API_URL}/appointments/calendar/${doctorId}`)
  if (!res.ok) throw new Error("Error cargando calendario del médico")
  return res.json()
}
