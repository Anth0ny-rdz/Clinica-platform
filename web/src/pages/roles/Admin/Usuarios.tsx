...
export default function Usuarios() {
  ...
  useEffect(() => {
    if (!puedeAsignarRol && roles.length > 0) {
      const pacienteRole = roles.find(r => r.name.toLowerCase().includes('paciente'))
      if (pacienteRole) {
  ...
  return (
    <div style={{ maxWidth: 600, margin: '2rem auto' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        <input type="tel" name="telephone" placeholder="Teléfono (10 dígitos)" value={form.telephone} onChange={handleChange} required />
      ...
      {message && <p style={{ color: message.startsWith('❌') || message.startsWith('⚠️') ? 'red' : 'green' }}>{message}</p>}
    </div>
  )
}
